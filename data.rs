...
pub fn identify_noisy_columns(records: &[Record], missing_threshold: f64) -> Vec<String> { 
    ...
    missing_counts
        .into_iter()
        .filter(|(_, count)| *count as f64 / total > missing_threshold)
        .map(|(key, _)| key)
        .collect()
}
// Encode categorical string data into numeric form (very hardcoding)
...
pub fn prepare_surgery_dataset(records: &[Record]) -> (DenseMatrix<f64>, Vec<f64>, Vec<String>) {
    ...
    let feature_keys: Vec<String> = selected_features()
        .into_iter()
        .filter(|k| k != &"type_of_breast_surgery" && !noisy_cols.contains(&k.to_string()))
        .map(|s| s.to_string())
        .collect();
